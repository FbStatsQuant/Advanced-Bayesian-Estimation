...
horseshoesp = function(y,X, method.tau = c("fixed", "truncatedCauchy","halfCauchy"), tau = 1,
                       method.sigma = c("fixed", "Jeffreys"), Sigma2 = 1, k=0.5,
                       burn = 1000, nmc = 5000, thin = 1, alpha = 0.05)
{
  ...
  if(algo==2)
  {
    Q_star = Matrix::crossprod(X) + Matrix::Diagonal(p, 1e-6)
  }
  for(i in 1:N){
    ...
    if(method.tau == "truncatedCauchy"){
      tempt = max(sum((Beta/lambda)^2)/(2*sigma_sq), 1e-8)
      et = 1/tau^2
      utau = stats::runif(1,0,1/(1+et))
      ubt_1=1
      ubt_2 = min((1-utau)/utau,n^(2 * k))
